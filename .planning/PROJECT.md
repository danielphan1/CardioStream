...
- [ ] Multi-dataset filtering & overlay — toggle any combination of BP / pulse / labs / incidents / procedures on or off (voice or click); selected types overlay together on one dashboard view. **Backend CRUD API complete** — Phase 7: Records Backend (Labs / Incidents / Procedures CRUD) (OVERLAY-01; Bearer-gated GET filtered + POST create for all three resources, 249 backend tests). **Manual-entry forms complete** — Phase 8: Manual-Entry Forms (OVERLAY-02; accessible Lab/Incident/Procedure forms, ≥48px targets, immediate confirmation without reload; code-reviewed with all critical/warning findings fixed same-session). The overlay UI itself (toggling + plotting multiple datasets on one timeline, by voice or click) remains — Phase 9.
...
---
*Last updated: 2026-08-21 — Phase 8: Manual-Entry Forms complete. Accessible Lab/Incident/Procedure forms (AddRecordPage + three field-set components) let caregivers submit new records with ≥48px targets, no drag/precision input, and immediate confirmation without a page reload (OVERLAY-02). Code review found 1 critical (async submit race causing silent data loss on mid-submit type-switch) + 3 warning issues; all 4 fixed and committed same-session (08-REVIEW-FIX.md) — the critical fix is concurrency-sensitive and has a manual spot-check pending. v1.0 MVP shipped & archived: dashboard, upload, auth gate, and deployment are live and verified private (SEC-03); the natural-language agent is built and verified in code but inert in production pending paid Claude API credits (deferred). v1.1 remaining: spoken replies (Phase 10), multi-dataset overlay UI (Phase 9 — the forms/backend it depends on are now both done), a full site guide (Phase 11), and an overall visual refresh (Phase 12).*
