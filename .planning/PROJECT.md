...
Not formally scoped as a named milestone. Two candidate directions exist: (1) Phases 15-18 (filtering + data-visual improvements — Unified Filter Surface, Trend Clarity & Chart Polish, Analytical Views, Deep Query), with Phase 15 ready to plan and Phase 18 blocked on funding the Anthropic API (AGENT-01); and (2) the Requirements → Deferred items below (activating the paid Claude API, agent-driven data entry). Run `/gsd-new-milestone` to formally name/scope either direction, or continue phase-by-phase without one.
...
- [x] Sitewide visual-identity replacement — new "Slack Water"/"Night Watch" nautical minimalist design-token layer, Inter + Space Grotesk type system, and a Two-Weight Rule (400/600 weight only, no 700-weight anywhere) applied across every screen and component, with zero regression to existing functionality or the accessibility floor — Validated in Phase 13: Visual Redesign — Nautical Minimalist Theme (D-01–D-09; 12/12 plans across 4 waves; 383/383 frontend tests green; code review found 1 critical + 4 warning + 2 info issues, all fixed same-session; explicit human sign-off obtained on a full cross-screen, cross-theme walkthrough in both light and dark theme — see `13-REVIEW.md`/`13-REVIEW-FIX.md` and `13-12-SUMMARY.md`)
- [x] Unified Show panel and combined timeline — one five-checkbox Show panel (BP/Pulse/Labs/Incidents/Procedures) drives one dual-axis combined chart (mmHg left axis / bpm right axis), auto-switching to an accessible dated Events list when no vitals are selected — a client-driven change request from Chris (2026-09-12) that explicitly reverses Phase 9's "no combined-metric chart" lock — Validated in Phase 14: Unified Show Panel and Combined Timeline (D-01–D-10; 6/6 plans across 3 waves; 468/468 frontend + 278/279 backend tests green, 1 pre-existing unrelated failure; live browser walkthrough confirmed all three client asks now work, including the two combinations previously impossible; the new `--line-pulse` token contrast-verified in both themes — see `14-VERIFICATION.md`). Caveat: the voice path (`show_only`/`toggle_dataset`) is unit-tested but not exercised against a live model — same AGENT-01 no-credits blocker as the base agent.
- [x] Guest Demo Mode — an isolated public portfolio demo behind its own guest username+password (separate Railway service + Postgres DB, not shared-DB row-tagging), boot-time auto-seeded from synthetic sample data, read-only for guests: the 4 mutating routes (upload, labs, incidents, procedures) reject guest tokens with a friendly 403, all GET routes and the `/agent` voice endpoint stay open — Validated in Phase 19: Guest Demo Mode (D-01–D-11; 7/7 plans across 3 waves; 27/27 code-level must-haves verified, 308 backend + 493 frontend tests green; the live demo deployment was stood up and human-verified end-to-end at https://health-dashboard-demo.vercel.app — guest login, demo badge, synthetic data of 132 readings + labs/incidents/procedures, write-route 403 confirmed via direct POST, Chris's real deployment confirmed unaffected — see `19-HUMAN-UAT.md` and `19-VERIFICATION.md`)
- [x] Unified filter surface — FilterBar's AM/PM and BP Category converted from single-select `aria-pressed` buttons to real multi-select checkboxes matching ShowPanel's control language, making "Stage 1 AND Stage 2" expressible for the first time; two new filters surfaced (Pulse Category, Time of Day, replacing AM/PM per an explicit Option-B planner decision) reaching data that already existed but had no UI path — Validated in Phase 15: Unified Filter Surface (25/25 must-haves verified across 8 plans/4 waves; 329 backend + 523 frontend tests green, `tsc -b --force` clean; code review 0 critical/4 warning/1 info; 2 items deferred to human/live testing — chip keyboard-nav/contrast spot-check and live voice round-trip (same pre-existing AGENT-01 funding gap) — see `15-HUMAN-UAT.md` and `15-VERIFICATION.md`)
- [x] Trend clarity & chart polish — a bold 7-reading rolling-average trend line (count-based window, not calendar-day) added to the combined timeline for systolic/diastolic/pulse, raw lines dimmed to a supporting role; readability pass on `CombinedTimeline`, `CategoryBars`, and `AmPmComparison` (responsive margins/gaps, no more clipped labels at narrow widths) — Validated in Phase 16: Trend Clarity and Chart Polish (12/13 must-haves verified across 3 verification rounds and 5 plans; round-2's BLOCKER — `CategoryBars`' right-margin never bounded against real container width, collapsing all six bars to zero-width on a 375px viewport — closed by round-3's `clampCategoryBarRightMargin()`/`truncateLabelForWidth()`, independently hand-verified against Recharts' own source by two separate agents; 558/558 frontend tests green, `tsc -b` clean; code review 0 critical/4 warning/2 info, all pre-existing carryover; 2 items approved by the user without a live-browser re-check — CategoryBars and AmPmComparison narrow-viewport rendering — recorded `partial` in `16-HUMAN-UAT.md` pending `/gsd-verify-work 16`)

### Active

No formally declared milestone wraps this work. v1.1 shipped 2026-08-27, and Phases 13, 14, 19, 15, and 16 ran afterward as client-driven continuation requests — a sitewide visual redesign, the combined-timeline reversal of Phase 9's lock, a public guest demo, the unified multi-select filter surface, and trend-line/readability chart polish — and are now validated above. The open continuation work is Phases 17-18 (data-visual improvements, client-driven request 2026-09-13; see ROADMAP.md for full per-phase scope): Phase 17 (Analytical Views) is not yet started; Phase 18 (Deep Query) is additionally blocked on funding the Anthropic API (AGENT-01). Run `/gsd-new-milestone` to formally name/scope a milestone around this work, or continue straight to `/gsd-plan-phase 17`.
