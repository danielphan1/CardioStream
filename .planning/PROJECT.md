...
---
*Last updated: 2026-08-26 — Phase 11: Full Site Guide / Instructions Tab complete. A Header "Guide" button (every authenticated view) opens a full-screen, text-only, one-scrollable-page guide covering the Command Bar, Filters, Charts, Overlay, Voice Replies, Upload, and Add a Record flows in a fixed what-it-does/by-click/by-voice format, plus a "What Can I Say" voice-command reference sourced from one shared module so it can never drift from the app's real command vocabulary. The guide is a landmark region mounted alongside the dashboard (never a view-swap) — CommandBar and its live mic session stay mounted, visible, and fully operable the whole time it's open. GUIDE-01 through GUIDE-04 all pass — 4/4 roadmap success criteria. Two verification passes ran inside this phase: the plan's own mandatory manual-verification checkpoint (performed via Claude-in-Chrome rather than deferred to the user) found and fixed a viewport-width-dependent content-overlap bug and a keyboard Tab-order gap; a follow-up code review then found and fixed 1 critical (focus dropped on open) and 2 related warnings (jump-link scroll clearance, no focus restoration on close). v1.0 MVP shipped & archived: dashboard, upload, auth gate, and deployment are live and verified private (SEC-03); the natural-language agent is built and verified in code but inert in production pending paid Claude API credits (deferred). v1.1 remaining: an overall visual refresh (Phase 12).*
